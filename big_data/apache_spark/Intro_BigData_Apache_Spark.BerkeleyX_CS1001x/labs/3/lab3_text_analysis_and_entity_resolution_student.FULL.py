...
def tokenize(string):
    ...
    """
    #return <FILL IN>
    #return list((set(simpleTokenize(string))-stopwords)) #not valid, unordered result
    return [word for word in simpleTokenize(string) if word not in stopwords]

#print "\n" , tokenize(quickbrownfox) # Should give ['quick', 'brown', ... 
...
# TODO: Replace <FILL IN> with appropriate code
amazonRecToToken = amazonSmall.map(lambda x: (x[0], tokenize(x[1]))) # <class 'pyspark.rdd.PipelinedRDD'>
googleRecToToken = googleSmall.map(lambda x: (x[0], tokenize(x[1]))) # <class 'pyspark.rdd.PipelinedRDD'>
#print googleSmall.take(1)
#print (googleRecToToken.take(2))
...
#print countTokens(googleRecToToken).take(20)
...
def findBiggestRecord(vendorRDD):
    ...
    """
    #return vendorRDD.takeOrdered(vendorRDD.count(), lambda x: -1 * len(x[1])) # return vendorRDD.count() = 200 elements
    return vendorRDD.takeOrdered(1, lambda x: -1 * len(x[1])) # return only 1 element


biggestRecordAmazon = findBiggestRecord(amazonRecToToken)
#print(type(biggestRecordAmazon))
#print(len(biggestRecordAmazon))
#print biggestRecordAmazon[0][0]
#print len(biggestRecordAmazon[0][1])
...
def idfs(corpus):
    ...
    #uniqueTokens = corpus.flatMap(lambda x: x[1]).distinct() # 4772  BAD
    uniqueTokens = corpus.flatMap(lambda x: set(x[1])) # 15144 OK
...
#cos_simil = cossim(testVec1, testVec1)
#print cos_simil
...
#print crossSmall.take(3)[0]
#print ""
#print crossSmall.take(3)[0][0]
#print ""
#print crossSmall.take(3)[0][1]
#print ""
...
def computeSimilarity(record):
    ...
    Returns:
        ...
        Note.- returns a tuple of tuples, one per each word.
    """
    googleRec = record[0]
    amazonRec = record[1]
...
#print""
#print goldStandard.take(2)[0]
#print ""
#print goldStandard.take(2)[1]
#print ""
#print goldStandard.count()
#print goldStandard.filter(lambda (x,y): y != 'gold').count()
...
#print similaritiesBroadcast.map(lambda(x,y,z): ((y+x),z)).take(1)
#print similaritiesBroadcast.map(lambda(x,y,z): ((y+" "+x),z)).take(1)
...
trueDupsRDD = (sims.join(goldStandard).map(lambda (x, (y,z)): (x,y)).cache())
#print trueDupsRDD.take(1)
#[('b0009jlux8 http://www.google.com/base/feeds/snippets/4923451909983851979', 0.03676255344374781)]
trueDupsCount = trueDupsRDD.count()

avgSimDups = trueDupsRDD.map(lambda s: s[1]).mean()


# nonDupsRDD = (sims.leftOuterJoin(trueDupsRDD).filter(lambda record: record[1][1] is None).cache())
# [('b000cc09uw http://www.google.com/base/feeds/snippets/18427202789383020961', (0.0, None))]
nonDupsRDD = (sims.leftOuterJoin(trueDupsRDD)
              .filter(lambda record: record[1][1] is None)
              .map(lambda (x, (y,z)): y)
              .cache())
...
# TODO: Replace <FILL IN> with appropriate code
amazonFullRecToToken = amazon.map(lambda x: (x[0], tokenize(x[1]))) # <class 'pyspark.rdd.PipelinedRDD'>
googleFullRecToToken = google.map(lambda x: (x[0], tokenize(x[1]))) # <class 'pyspark.rdd.PipelinedRDD'>
...
#print amazonWeightsRDD.take(3)[0][0]
#print amazonWeightsRDD.take(3)[0][1]
...
def invert(record):
    ...
    """
    pairs = list((v, record[0]) for v in record[1].keys())
    return (pairs)
...
commonTokens = (amazonInvPairsRDD
                ...
                .map(swap)
                #.groupByKey().mapValues(list) #very slow solution (2.8 minutes) due to ".groupByKey().mapValues(list)"
                .map(lambda (x,y): (x, [y])).reduceByKey(lambda a,b: a+b) #slow too (in one computer), better if more?
                .cache())
...
#print "4a (amazon_id, token list) and (google_url, token list) "
#print amazonFullRecToToken.take(1)
#print googleFullRecToToken.take(1)
#print amazonInvPairsRDD.count()
#print googleInvPairsRDD.count()
#print ""
...
#print googleWeightsRDD.take(1)
#print ""
#print "4e ((amazon_id, google_url), (token list)) "
#print commonTokens.take(12)[0]
#print commonTokens.take(12)[10]
#print commonTokens.take(12)[10][0][0]
#print commonTokens.take(12)[10][0][1]
#print commonTokens.take(12)[10][1]
...
def fastCosineSimilarity(record):
    ...
    Args:
        record: ((ID, URL), token)
        i.e. token = ['hours', 'game', 'll', 'encore', 'classic', 'software']
    Returns:
        pair: ((ID, URL), cosine similarity value)
        
    WARNING Make sure you use broadcast variables you created for both the weights and norms
    """
    amazonRec = record[0][0]
    googleRec = record[0][1]
    tokens = record[1]
    
    # ... and computes the sum for each of the tokens in the token list of the products 
    # of the Amazon weight for the token times the Google weight for the token ...
    # 
    # amazonWeightsBroadcast.value.items() = {'b0009mg80a': {'control': 0.06805807622504537 ...
    # amazonWeightsBroadcast.value['b0009mg80a'] = {'control': 0.06805807622504537 ...

    # ERROR "if k in tokens" missed here bellow
    # s = sum((googleWeightsBroadcast.value[googleRec].get(k, 0.0) * v) \
    #     for k,v in amazonWeightsBroadcast.value[amazonRec].items())
    s=0.0
    for k,v in amazonWeightsBroadcast.value[amazonRec].items():
        if k in tokens:
            s+= v * googleWeightsBroadcast.value[googleRec].get(k, 0.0)
            
    # ... The sum should then be divided by the norm for the Google URL 
    # and then divided by the norm for the Amazon ID.
    value = s
    value /= googleNormsBroadcast.value[googleRec]
