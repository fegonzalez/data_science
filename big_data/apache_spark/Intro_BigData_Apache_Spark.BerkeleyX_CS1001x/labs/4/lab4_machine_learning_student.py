...
def sortFunction(tuple):
    ...
    """
    key = unicode('%.3f' % tuple[0]) # return str  (unicode type)
    value = tuple[1]
    return (key + ' ' + value)
...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    """
    fsum = float(sum(IDandRatingsTuple[1]))
    return (IDandRatingsTuple[0],             (len(IDandRatingsTuple[1]), fsum/len(IDandRatingsTuple[1])))
...
print getCountsAndAverages((100, (10.0, 20.0, 30.0)))
print getCountsAndAverages((110, xrange(20)))
...
# validationRDD is a tuple of tuples (userID, movieID, rating)
# validationRDD.take(1) # [(1, 1287, 5.0)]
...
# trainingRDD is a tuple of tuples (userID, movieID, rating)
#trainingRDD.take(1) # [(1, 914, 3.0)]
...
myRatingsRDD_list =  sc.parallelize(myRatedMovies).map(lambda x: x[1]).take(myRatingsRDD.count()) 
myUnratedMoviesRDD = (moviesRDD.map(lambda x: (myUserID, x[0]))
                      .filter(lambda x: x[1] not in myRatingsRDD_list))
...
#  into an RDD of the form (MovieID, number of ratings)
...
# Use RDD transformations with PredictedWithCountsRDD and moviesRDD (Movie ID, tittle), 
# to yield an RDD with tuples of the form 
