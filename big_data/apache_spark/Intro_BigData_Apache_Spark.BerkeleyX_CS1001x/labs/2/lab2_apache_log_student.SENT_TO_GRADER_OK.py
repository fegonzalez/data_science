...
_SUCCESS_CODES = xrange(200, 207)

not200 = (access_logs
          .filter(lambda log: log.response_code not in _SUCCESS_CODES))
...
# pair(day, host), pairs can be repeated
dayToHostPairTuple = access_logs.map(lambda log: (log.date_time.day, log.host))
# pair(day, host)), unique pairs
dayGroupedHosts = dayToHostPairTuple.distinct()
#pair(day, #host), distint hosts
dayHostCount = dayGroupedHosts.groupByKey().mapValues(len)

#ordered pairs
dailyHosts = dayHostCount.sortByKey().cache()

#list of the first 30 elements
dailyHostsList = dailyHosts.take(30)
print 'Unique hosts per day: %s' % dailyHostsList
assert(dailyHosts.is_cached)
...
dayAndHostTuple = access_logs.map(lambda log: (log.date_time.day, log.host))  # (day, host), with repeated pairs
groupedByDay = dayAndHostTuple.groupByKey().mapValues(len)   # (day, #request)
#sortedByDay = groupedByDay.sortByKey()  # (day, #request), sorted by day

# number of requests per host per day = unique_host_each_day
# unique_host_each_day = list [unique_host_day_1, unique_host_day_2, ...]
# pair(day, host)), unique pairs
unique_host_each_day = dayAndHostTuple.distinct().groupByKey().mapValues(len)#.sortByKey()

#avgDailyReqPerHost = sortedByDay.join(unique_host_each_day).map(lambda (x,y): (x, y[0]/y[1])).sortByKey().cache()
avgDailyReqPerHost = groupedByDay.join(unique_host_each_day).map(lambda (x,y): (x, y[0]/y[1])).sortByKey().cache()

avgDailyReqPerHostList = avgDailyReqPerHost.take(30)
print 'Average number of daily requests per Hosts is %s' % avgDailyReqPerHostList
assert(avgDailyReqPerHost.is_cached)
...
topErrDate = errDateSorted.takeOrdered(5, lambda x: -x[1]) # -x == descending order
